class CopyToClipboard {
  constructor(snippet) {
    ...
    window.requestAnimationFrame(() => {
      ...
      this.snippet.appendChild(button);
      button.addEventListener('click', this.onClick.bind(this));
    });
  }

  async onClick() {
    const textToCopy = this.snippet.querySelector('code').innerText;
    await navigator.clipboard.writeText(textToCopy);
    this.snippet.classList.add('--success');
    setTimeout(() => {
      this.snippet.classList.remove('--success');
