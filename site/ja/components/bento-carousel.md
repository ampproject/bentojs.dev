...
permalink: "/ja/components/bento-carousel/"
short_title: Carousel
layout: layouts/component.njk
description: 似たようなコンテンツピースを縦軸または横軸に沿って表示するための汎用カルーセルです。
---
...
似たようなコンテンツピースを縦軸または横軸に沿って表示するための汎用カルーセルです。コンポーネント直下の子要素は、カルーセルのアイテムとみなされます。それらの各ノードも、任意の子要素を持つことができます。カルーセルは任意の数のアイテムと、アイテムを 1 つずつ先送りまたは巻き戻すためのオプションのナビゲーション矢印で構成されます。カルーセルは、ユーザーがスワイプするか、カスタマイズ可能な矢印ボタンを使用すると、アイテム間を移動します。

<div class="bd-usage bd-card bd-card--light-sea-green">
<p>bento-base-carousel をウェブコンポーネントまたは React 関数コンポーネントとして使用します。</p> <a href="#web-component" class="">↓ ウェブコンポーネント</a> <a href="#preact%2Freact-component">↓ React / Preact</a>
</div>
...
Bento 対応のコンポーネントには、API を通じた高いインタラクティブ性があります。`bento-base-carousel` コンポーネント API にアクセスするには、次のスクリプトタグをドキュメントに含めます。
...
スライドのサイズは、<code>mixed-lengths</code> を<strong>指定しない</strong>場合に、カルーセルによって自動的に設定されます。
